...
grid = [[] for _ in range(maxy + 1)]
for y in range(maxy + 1):
    for x in range(maxx - minx + 1):
        if y == 0 and x == rel_sand_x:
            grid[y].append("+")
        else:
            grid[y].append(".")
    ...
    while not is_resting:
        try:
            if grid[sy + 1][sx] not in ("#", "o"):  # Falling down
                ...
                if grid[sy + 1][sx - 1] == ".":  # attempt one step down and left
                    sy += 1
                    sx -= 1
...
while True:
    previous_resting_counter = resting_counter
    calculate_sand(grid, rel_sand_x, 1)
    if resting_counter <= previous_resting_counter:  # not growing
        break

draw_grid(grid)
print("Part 1: ", resting_counter)
